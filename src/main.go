...
func main() {
	if _, err := aql.Parse("FOO USER"); err != nil {
		log.Println(err)
	}
	if _, err := aql.Parse("ADD"); err != nil {
		log.Println(err)
	}
	if _, err := aql.Parse("ADD USER foo TO ROLE"); err != nil {
		log.Println(err)
	}
	if _, err := aql.Parse("ADD USER foo ROLE ROLE bar"); err != nil {
		log.Println(err)
	}
	if _, err := aql.Parse("ADD USER foo TO TO bar"); err != nil {
		log.Println(err)
	}
	if _, err := aql.Parse("ADD USER 'foo' TO ROLE bar"); err != nil {
		log.Println(err)
	}
	if _, err := aql.Parse("ADD USER foo TO ROLE 'bar'"); err != nil {
		log.Println(err)
	...
	// aql.Parse("GET name, json, age FROM foo.bar WHERE name = \"john carter\" AND json = {\"hello\": \"john carter\"}")
}
